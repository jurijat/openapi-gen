...
export async function callLlmApi(config: AppConfig, prompt: string, temperature: number): Promise<string> {
  if (!config.apiKey) {
    throw new Error('API Key is missing. Please provide AI_API_KEY in .env or via --api-key.');
  }
  ...
  try {
    ...
    const response = await axios.post<LlmApiResponse>(
      apiUrl,
      {
        model: config.apiBaseModel || "gpt-4o", // Or make this configurable? For now, hardcode a reasonable default.
...
// Add type annotations for parameters and return type
export async function planFileStructure(config: AppConfig, repomixContent: string, detectedFramework: string): Promise<string[]> {
  ...
  if (repomixContent.length > maxRepomixLength) {
      console.warn(`Warning: Repomix content truncated to ${maxRepomixLength} characters for LLM planning prompt.`);
      truncatedRepomix = repomixContent.substring(0, maxRepomixLength);
  }


  const prompt: string = `Based on the following ${detectedFramework} codebase snippet:\n\n\`\`\`\n${truncatedRepomix}\n\`\`\`\n\nPropose a list of relative file paths for YAML files required to represent the complete OpenAPI 3.x schema using $ref. The main file must be '${path.join(config.outputDir, config.outputFile)}'. Organize component files logically within the '${config.outputDir}' directory (e.g., '${config.outputDir}/components/schemas/User.yaml', '${config.outputDir}/paths/users.yaml'). Provide *only* the list of relative file paths, one path per line, starting with the main file. Do not include any other text, explanations, or formatting.`;
  ...
  try {
    ...
    const responseContent: string = await callLlmApi(config, prompt, config.initialTemperature);
    ...
    const plannedFiles: string[] = responseContent
      ...
      .filter((line: string) => line.length > 0 && (line.endsWith('.yaml') || line.endsWith('.yml'))); // Add type for line, Basic validation
    ...
    if (!plannedFiles.includes(mainFilePath)) {
        console.warn(`Warning: LLM plan did not include the main file '${mainFilePath}'. Adding it to the beginning.`);
        plannedFiles.unshift(mainFilePath);
    } else if (plannedFiles[0] !== mainFilePath) {
        console.warn(`Warning: Main file '${mainFilePath}' was not first in the LLM plan. Reordering.`);
        plannedFiles.sort((a: string, b: string) => a === mainFilePath ? -1 : b === mainFilePath ? 1 : 0); // Add types for a, b
    }
