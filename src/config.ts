...
export function loadConfig(argv: Argv): AppConfig {
  ...
  const config: AppConfig = {
    // API details: Prioritize CLI args, then .env, then null
    apiKey: argv.apiKey || process.env.AI_API_KEY || null,
    apiBaseUrl: argv.apiBase || process.env.AI_BASE_URL || null, // Ensure this matches your .env variable name
    apiBaseModel: argv.apiBaseModel || process.env.AI_BASE_MODEL || null,
  ...
  if (!config.apiKey) {
    console.warn('Warning: AI_API_KEY not found in .env file or provided via --api-key.');
    // Depending on whether the LLM step is reached, this might become an error later.
  }
   if (!config.apiBaseUrl) {
    console.warn('Warning: AI_API_BASE not found in .env file or provided via --api-base.');
